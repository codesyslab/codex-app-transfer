...
>
> 其他已内置的 chat-completions 兼容供应商(包括 **DeepSeek、Kimi(月之暗面)、Xiaomi MiMo(Pay for Token)、智谱 GLM、智谱 GLM Coding、GLM（Z.ai）(zai-login)、GLM（BigModel）(bigmodel-login)、阿里云百炼(API Key / Token Plan)、MiniMax、OpenCode Go、WorkBuddy(腾讯 CodeBuddy;模型选择器显示 Credit 折扣倍率)、QoderWork CN(阿里 Qoder;模型选择器显示 Credit 折扣倍率)**)**未做长期真机回归**,仅停留在单元测试 + 偶发用户反馈层面。
>
...
- 思考过程(reasoning)在新版 Codex Desktop 正常显示:reasoning 流走 **summary 通道**(`reasoning_summary_text.delta`,实测官方 gpt-5.5 @ v26.623 即此单通道形态);早期 MOC-203 曾双发 content 通道兼容,后实测双发导致同一思考块重复更新、流式期间整段闪烁,已收敛回单通道(MOC-293)。chat 路径同时修复 reasoning 与 tool_call 流式事件交错(开工具前先闭合 reasoning)、gemini 修复工具调用折叠(`functionCall` 后空 text part 不再产生空 message item,同轮工具正常折叠成组)(MOC-203)
- **任务完成后过程自动折叠(对齐官方 GPT 行为,MOC-293 / MOC-295)**:走 Chat Completions 转换的 provider(如 GLM / Kimi / DeepSeek / WorkBuddy)整轮处理过程(思考 + 工具调用 + 铺垫消息)在最终答复出来后自动折叠成 "Worked for Ns",只展开最终回复——与官方 ChatGPT 账号一致。实现:三条转换路径(chat / gemini_native / anthropic_messages)给 assistant message item 补官方 wire 的顶层 `phase` 字段(工具轮铺垫 → `commentary` 归入折叠过程区,最终答复 → `final_answer` 展开);流式 `output_item.added` 一律先发临时 `commentary`,message 的 `output_item.done` 延迟到收尾侧(已知 stop_reason / has_seen_tool_calls)再带权威 phase emit——chat 路径 message 只在流末闭合天然权威,gemini / anthropic 路径在收尾侧 emit_terminal / emit_completed 时按终态信号(anthropic `final_stop_reason` / gemini `has_seen_tool_calls`)定 phase,保证折叠只在真最终答复收尾时触发一次、任务中途不抖动。注:最终答复流式期间先显示在过程区、收尾时提升为最终回复(第三方模型无法像 GPT 那样预先声明消息通道)。grok_web 与 Responses 直通不经此转换、不涉及
...
- **apply_patch 中间层(格式恢复)**:第三方 chat 模型无 GPT 的 lark 语法约束生成,常产出畸形 V4A(双边 `@@`、Add File 漏 `+`、上下文 byte 失配、缺 `*** Begin/End Patch` 信封、漏空行、漏前缀、**多个不连续 hunk 漏 `@@` 分隔**等)。中间层按**白名单逐条恢复**成合法格式再发 Codex —— 读盘把 `@@` 锚点 / 上下文对齐成文件真实字节、补回漏写空行、空文件 / 空 rename 转 `Delete+Add`、**多区编辑漏 `@@` 时按文件真实位置自动切段插裸 `@@`**(MOC-263 P0:仅唯一可分段才动,浮动 `+` 落点歧义则透过)等;**非破坏性**(不丢内容、不覆盖)、**未知一律原样透过**(交 Codex 报错让模型自纠,不猜)。读盘解析的 cwd 用**最近若干 cwd 候选**而非单一全局值,使并发多会话下兜底不再被别项目的 cwd 覆盖失效(MOC-263 P1)。针对模型给 Add File 终止符误加 `+`(`+*** End Patch`,逐行加 `+` 时的常见错)导致新建文件残留一行 `*** End Patch`:根治在注入指引(教模型终止符不加前缀);中间层遇这种带前缀的歧义终止符**按文件类型消歧** —— 代码 / 结构化配置文件(裸 `*** End Patch` 不可能是合法源码末行)直接剥成裸终止符;文档 / 文本类无法排除是正文最后一行,则不猜、留 incomplete 让模型按指引重发(避免误删正文)。同时优化 chat 路径 apply_patch 指引(context 必须是已存在行 / 序列编辑别重删已删行 / memory 文件专属引导)(MOC-268)。思路镜像 Codex 给 GPT 用的 V4A lark 语法约束、在 chat 路径事后保证(致谢 [openai/codex](https://github.com/openai/codex) 的 apply_patch lark grammar)(MOC-194 / MOC-263 / MOC-268)
...
- chat-completions 兼容 provider(DeepSeek / Kimi / MiMo / GLM / 阿里云百炼 / MiniMax 等)上游返 4xx/5xx 时同款对齐:此前 proxy 原样透传 HTTP 错误状态 + JSON error body,Codex APP 期待 SSE 流而**卡 "Thinking"**(既不报错也不重试,无法进下一轮);现改写成合规 `response.failed` 流,400 请求错误 / 401 鉴权 / 403 权限 → `invalid_prompt`(永久,surface + 停手),429 瞬时限流(per-minute)/ 5xx / 超时等保留可重试语义;**429 配额 / 使用窗口耗尽**(保守强信号检测,如 GLM「已达到 N 小时的使用上限,将在 … 重置」)识别为永久态 → fail-fast,原样显示上游额度提示(含重置时间)+ 停手,不再反复「Reconnecting」重连(顺带消除重连触发的「proxy adapter error」误报);与 grok / gemini 同走 `codex_retry_code` 白名单(MOC-103 / MOC-264)
...
- **导入 / 恢复其他工具留下的隔离会话(CAT-255)**:Codex Desktop 会话列表按当前活动 `model_provider` 过滤显示。cc-switch 等工具往 `config.toml` 写了第三方 `model_provider`,其留下的旧会话在 transfer(本项目刻意不写 model_provider,锚定 `openai` 以与官方互通)视图下被隐藏。设置页 → 高级 → 会话历史新增「**导入**」(把第三方会话就地归一成 `openai`,在 transfer 里重新显示)与「**恢复**」(把选中会话的 `model_provider` 写回某第三方值,让对应工具重新看到——导入的逆操作);transfer 启动检测到隔离会话时也弹窗提示导入。两者均需先关闭 Codex(写它独占的会话状态库 `state_<N>.sqlite`),确认后自动关闭 → 就地改写 → 重启 Codex
...
- **真实 ChatGPT 账号 Plugins 解锁**(relay 模式,v2.2.0):用真实账号而非 CDP 伪造登录态解锁 Codex Plugins —— 应用内调起官方 `codex login`(账号开关统一走下方「插件解锁三态」选择器,MOC-257)。relay 保留 `auth_mode=chatgpt` + tokens 让 Codex **原生**显示 Plugins 入口、消除 CDP 伪造的启动高延迟;第三方模型经 `openai_base_url` 走 proxy,账号 / 插件 backend 经 `chatgpt_base_url` 透传真 chatgpt.com。transfer **不刷新** single-use refresh token(与本机 Codex 双刷会 `refresh_token_reused` 烧号),刷新只归源头(本机 Codex 自刷 / `codex login` 自取)。配套**系统代理连通检测**:仪表盘「网络代理」状态卡 + 解锁 gate(账号有效 AND 系统代理可达才解锁,缺则引导开代理 / 登录 / 强制),探测只连代理端口、不碰 chatgpt.com。另:relay 下 transfer 不刷新 token、靠源头保鲜,但服务端撤销(双刷烧号 / 别处重登)本地 JWT `exp` 看不到 → 此前前端因本地 token 没到期而**误显账号正常**;现 proxy 透传探测到 chatgpt backend **401** 即回灌「需重新登录」状态,前端及时提示重登(MOC-104 / MOC-114 / MOC-124)
...
- **供应商编辑页 UX 改进**:① 点「获取模型」时自动过滤走 `/messages`(Anthropic)端点的模型(如 OpenCode Go 中的 MiniMax/Qwen 系),按钮左侧持久显示「messages 协议模型已过滤」提示,避免误选不兼容端点的模型;② API Key 输入框加眼睛按钮(显示 / 隐藏切换),方便核查填入的 key;③ 内置供应商预设(含 OpenCode Go)显示各自官方图标
...
| WorkBuddy(腾讯 CodeBuddy,`copilot.tencent.com/v2`,openai_chat) | 🧪 | 🧪 | 🧪 | 复用桌面端登录 token(~1 年期 Keycloak Bearer)直连,纯 bearer 无需额外头;协议走通用 openai_chat adapter(同 DeepSeek/MiniMax);出站自动伪装成桌面端 coding 模式 wire 指纹(X-Agent-Intent: coding + X-IDE-* + OpenAI SDK UA/X-Stainless-* + 每请求 X-Conversation-*,见 forward.rs `injects_workbuddy_source_headers`)避风控;**当前仅冒烟(单轮直连各模型回包正常,含完整指纹头实测服务端接受),多轮/autocompact/tool 修复长期真机回归待补**。实测可用 id:deepseek-v4-pro/flash、glm-5.2/5.1/5v-turbo、minimax-m3、kimi-k2.7/k2.6、hy3-preview、auto;渠道(如中国移动 Token Plan)开通子集不同,Claude(default-1.x)等可能未开通。**两种接入路**:`workbuddy`(粘 token)+ `workbuddy-login`(账号登录 OAuth,`authScheme=workbuddy_oauth`,点登录走 `/v2/plugin/auth/{state,token}` 外链轮询拿凭证 + 自动 refresh,免手抠 token;账号登录端到端需真机验证)。**`workbuddy-login` 单 provider 多账号池 + 配额守护自动切换(MOC-291)**:同一 `workbuddy-login` provider 下可登录多个账号组成池,每账号**独立设备指纹**(`X-Device-Id` per-account,避免网关风控关联);账号列表显示脱敏手机号(如 `183****5600`)替代不可读 uid,可手动「设为当前」/「移除」。**配额守护**:后台每 ≥60s 查各账号 `get-user-resource` 剩余积分,某账号总剩余 < 阈值(默认 20,设置项可调)即标记耗尽、自动 sticky 切到下一个有额度账号,对 Codex 透明(独立于额度面板开关,面板关也跑);全耗尽时兜底选最快解禁的账号。老单文件 `workbuddy-oauth.json` + 全局 `workbuddy-device-id` 首次启动自动迁移为池首账号(无缝) |
...
| **Kimi / Kimi Code / GLM / 阿里云百炼 / Xiaomi MiMo / MiniMax / OpenCode Go** | 不传字段 | 不传字段 | 上游不认 `reasoning_effort`,用自家默认 thinking;如需控制在 `requestOptions` 写 provider-native 字段 |
...
**Codex 26.623+ 显示档位需补齐启用集(MOC-285)**:Codex(codex-cli 0.142.3)起新增用户级持久设置 `enabled-reasoning-efforts`(默认 `low/medium/high/xhigh/ultra`,不含 `none`/`max`,且无可见开关),reasoning 选择器实际显示 = 模型档位 ∩ 该启用集 —— 导致上面给 GLM 等写的 `none`/`max` 升级到该版本后被过滤、档位塌成「Medium」。本项目在每次启动 Codex 前自动把 `reasoning_tiers` 表暴露**且 Codex 默认隐藏**的档位(`none`/`max`)并入 `~/.codex/.codex-global-state.json` 的 `enabled-reasoning-efforts` 持久 atom(**只增隐藏档、从不删档、也不回填用户删过的默认可见档如 `high`、不动用户其它设置**;`high` 等默认可见档由 Codex 自身默认集提供。因选择器是「模型档 ∩ 启用集」,补全启用集对每个模型仍只显其声明档,GPT 仍 `low/medium/high/xhigh`、GLM 才出现 `none/max`,零副作用),还原原生两档显示。
...
### apikey 模式间歇性报 `missing or invalid gateway api key` / 偶发 401

apikey 模式下 gateway key(`cas_` 开头)同时写在 proxy 配置和 Codex `auth.json`(`OPENAI_API_KEY`)两处;proxy 重启而 Codex Desktop 进程未重启时,两边可能短暂不一致(proxy 用新 key、Desktop 仍发旧 key),导致间歇性 401(#546)。**已修**:gateway 校验从精确匹配改为 `cas_` 格式兜底 —— 只要 incoming token 是 `cas_` 格式就放行,无论双写是否一致。gateway 的定位是防止外部程序误连 proxy(真安全在本地文件权限),`cas_` 格式足以区分"被 transfer 配置过的 Codex"与"不相干的外部请求"(#546)。
...
- **存储**:`~/.codex-app-transfer/config.json`(配置,与 v1.x 互通)、`~/.codex-app-transfer/sessions.db`(L2 sqlite 会话持久化)、`~/.codex-app-transfer/blobs/`(会话内大图按 sha256 去重外置,删 db 不会自动清,需一并删或走「设置 → 高级 → 会话历史 →『清空会话历史』」:把全部 Codex 对话 rollout 移到回收站(可恢复)+ 清空该 L2 缓存)、`~/.codex/{config.toml,auth.json,.credentials.json}`(Codex APP 集成)、`~/.codex-app-transfer/mcp-credentials.json`(MCP 凭据镜像,在 `~/.codex` 之外)
...
- [`BlockedPath/pi-xai-oauth`](https://github.com/BlockedPath/pi-xai-oauth) — grok 登录反向工程参考
- [`galaxywk223/codex-plugin-unlocker`](https://github.com/galaxywk223/codex-plugin-unlocker) — Plugins 解锁注入脚本
- [`obra/superpowers`](https://github.com/obra/superpowers) — 强约束 skill 插件内置
